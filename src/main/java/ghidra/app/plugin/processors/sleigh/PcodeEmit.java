...
public abstract class PcodeEmit {
	...
	void updateOverlaySpace(VarnodeData[] in, int isize, VarnodeData out) {
		Reference ref = ((InstructionPcodeOverride)override).getPrimaryOverridingReference();
		
		if (ref == null) {
			return;
		}
		
		AddressSpace origSpace = ref.getFromAddress().getAddressSpace();
		AddressSpace destSpace = ref.getToAddress().getAddressSpace();
		
		for (int i = 0; i < isize; ++i) {
			VarnodeData dest = in[i];

			if (dest.space.equals(origSpace)) {
				dest.space = destSpace;
			}
		}
		
		if (out != null) {
			if (out.space.equals(origSpace)) {
				out.space = destSpace;
			}
		}
	}
